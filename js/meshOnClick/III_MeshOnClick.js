...
export default class III_MeshOnClick
{
    //#region 
    //system props
    // animation props
    ...
    {
        camera = new III_Cam({x:6,y:6,z:-3});
    ...
    rayCastPlane()
    {
        this.#NormalMouse = new THREE.Vector2();//normalized cursor position
        this.#IntersectionPoint = new THREE.Vector3();// mouse click position
        this.#PlaneNormal = new THREE.Vector3();//plane direction
        const plane = new THREE.Plane();//cast object for ray casting
        const raycaster = new THREE.Raycaster();

        window.addEventListener('mousemove', e =>{
            //finding and normalize cursor coordinates
            this.#NormalMouse.x = (e.clientX/ window.innerWidth) * 2 - 1;
            this.#NormalMouse.y = -(e.clientY/ window.innerHeight) * 2 + 1;

            //update plane position
            this.#PlaneNormal.copy(camera.position).normalize();
            plane.setFromNormalAndCoplanarPoint(this.#PlaneNormal, 
                                                scene.position);
            //emit ray and cast intersection with plane.
            raycaster.setFromCamera(this.#NormalMouse,
                                    camera);
            raycaster.ray.intersectPlane(plane,this.#IntersectionPoint);

            console.log('uwu');
        });

        window.addEventListener('click',e =>{
            const s = this._fresnel_bubble('metalic',{
                position: this.#IntersectionPoint
            })
        });
    }
    ...
    /**
     * Create a Sphere with fresnel shader and reflections.
     * 
     * @param {'transparent' | 'metalic'} type
...
// export {III_MeshOnClick as III_GsapCam};